...
fn qmult(p: vec4f, q: vec4f) -> vec4f {
    let pv = p.xyz; let qv = q.xyz;
    return vec4f(p.w * qv + q.w * pv + cross(pv, qv), p.w * q.w - dot(pv, qv));
}

fn qaxisangle(axis: vec3f, phi: f32) -> vec4f {
    return vec4f(sin(phi * .5) * axis, cos(phi * .5));
}

fn qmouse(pos: vec2f) -> vec4f {
    let PI = 3.1415927;
    var p = pos;
    if (mouse.click <= 0) { p = vec2f(-.35,.6); }
    let rotY = qaxisangle(vec3f(0., 1., 0.), PI - 2. * PI * p.x);
    let rotX = qaxisangle(vec3f(1., 0., 0.), .5 * PI - PI * p.y);
    return qmult(rotY, rotX);
}

fn rotate(p: vec3f, rotor: vec4f) -> vec3f {
    let rv = rotor.xyz;
    return qmult(rotor, vec4f(p * rotor.w - cross(p, rv), dot(p, rv))).xyz;
}

fn map(p: vec3f) -> f32 {
    let t = i32(time.elapsed % 32.0);
    var d: f32;
    if      (t == 0)  { d = sdSphere(p, 1.); }
    else if (t == 1)  { d = sdEllipsoid(p, vec3f(1.3, .8, .9)); }
    else if (t == 2)  { d = sdBox(p, vec3f(.7)); }
    else if (t == 3)  { d = sdRoundBox(p, vec3f(.5), .2); }
    else if (t == 4)  { d = sdBoxFrame(p, vec3f(.7), .1); }
    else if (t == 5)  { d = max(sdBox(p, vec3f(.8)), .5 * sdGyroid((p+vec3f(2.)) * 6.5, .2) / 6.5); }
    else if (t == 6)  { d = sdTorus(p, 1.1, .3); }
    else if (t == 7)  { d = sdCappedTorus(p+vec3f(0.,.5,0.), 1.1, .2, vec2f(sin(1.3), cos(1.3))); }
    else if (t == 8)  { d = sdLink(p+vec3f(0.,.1,0.), .4, .8, .1); }
    else if (t == 9)  { d = sdVerticalCapsule(p + vec3f(0.,.45,0.), 1.3, .3); }
    else if (t == 10) { d = sdCapsule(p, vec3f(-0.7), vec3f(0.7), 0.3); }
    else if (t == 11) { d = sdCylinder(p, vec3f(-0.7), vec3f(0.7), 0.3); }
    else if (t == 12) { d = sdVerticalCylinder(p, .9, .3); }
    else if (t == 13) { d = sdRoundedCylinder(p, .9, .3, .3); }
    else if (t == 14) { d = sdInfiniteCylinder(p, vec3f(.0,.0,.5)); }
    else if (t == 15) { d = sdCone(p + vec3f(.0,-.9,.0), 1.3, vec2f(sin(.6), cos(.6))); }
    else if (t == 16) { d = sdConeBound(p + vec3f(.0,-.9,.0), 1.3, vec2f(sin(.6), cos(.6))); }
    else if (t == 17) { d = sdInfiniteCone(p+vec3f(.0,-.9,.0), vec2f(sin(.6), cos(.6))); }
    else if (t == 18) { d = sdCappedVerticalCone(p, .4, .9, .5); }
    else if (t == 19) { d = sdCappedCone(p, vec3f(.0), vec3f(.5), .9, .5); }
    else if (t == 20) { d = sdRoundVerticalCone(p+vec3f(.0,.4,.0), .9, .6, .3); }
    else if (t == 21) { d = sdRoundCone(p, vec3f(.0), vec3f(.6), .6, .3);; }
    else if (t == 22) { d = sdSolidAngle(p + vec3f(.0,.8,.0), vec2f(sin(.6), cos(.6)), 1.3); }
    else if (t == 23) { d = sdPlane(p, normalize(vec3f(0.,1.,-.5)), 2.); }
    else if (t == 24) { d = sdOctahedron(p, 1.2); }
    else if (t == 25) { d = sdOctahedronBound(p, 1.2); }
    else if (t == 26) { d = sdPyramid(p + vec3f(.0,.3,.0), 1.3); }
    else if (t == 27) { d = sdHexPrism(p.zxy, vec2f(1., .5)); }
    else if (t == 28) { d = sdTriPrism(p.zxy, vec2f(1., .5)); }
    else if (t == 29) { d = udTriangle(p, vec3f(-.9, 0., 0.), vec3f(.6, .8, .8), vec3f(.6, -.6, .0)) - .00001; }
    else if (t == 30) { d = udQuad(p, vec3f(-.7), vec3f(.5, 0., .5), vec3f(.7), vec3f(-.5, 0., -.5)) - .00001; }
    else if (t == 31) { d = sdBunny(p / 1.5) * 1.5; }
    return d;
}
...
fn main_image(@builtin(global_invocation_id) id: uint3) {
    ...
    let fragCoord = vec2f(float(id.x) + .5, f32(screen_size.y - id.y) - .5);

    let AA = 3.;
    var col: vec3f;

    for (var i: f32 = 0.; i < AA; i = i + 1.) {
    for (var j: f32 = 0.; j < AA; j = j + 1.) {
        let dxy = vec2<f32>(i, j) / AA;
        let uv = (2.*fragCoord - vec2f(screen_size) + dxy) / f32(min(screen_size.x, screen_size.y));
        var ro = vec3f(0., 0., 3.2);
        var rd = normalize(vec3f(uv, -2.));

        let rot: vec4f = qmouse(vec2f(mouse.pos) / vec2f(screen_size) + vec2f(-.5,0.0));
        rd = rotate(rd, rot);
        ro = rotate(ro, rot);
